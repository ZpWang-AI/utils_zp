...
- 飞书文档 link 和本地文件，更新时读取和写入 `agent_zp/.feishu_link.agent.yaml`。
  - **注意不要覆盖掉评论！**
- 文档定义：
  - `README.agent.md`：面向项目开发者和 agent，最常用，用来记录所有规则和细节。
  - `README.md`：面向技术人员，基于 `README.agent.md`，用来说明重要信息。
  - `leader_summary.md`：面向非技术人员，用来汇总展示主要结果、重要说明和核心过程。
    - 第一段 `TL;DR`：目标、结论、下一步。
    - 后续正文论述，重要图表往前放，最后分析与总结。
